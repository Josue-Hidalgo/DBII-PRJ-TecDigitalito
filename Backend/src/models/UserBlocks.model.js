 ...
 * Redis expira la key automáticamente al vencer el TTL → desbloqueo sin intervención.
 * @param {string} userId
 * @param {string} reason  - 'max_intentos_fallidos' | 'sospecha' | 'manual'
 * @param {string} ip      - IP que disparó el bloqueo (para logs en Cassandra)
 * @param {number} [ttl]   - Segundos de bloqueo; usa BLOCK_TTL si no se especifica
 */
const blockUser = async (userId, reason = 'max_intentos_fallidos', ip = 'unknown', ttl = BLOCK_TTL) => {
...
/**
 * Verifica si un usuario está bloqueado (HU-02 → mostrar "Credenciales inválidas").
 * @param {string} userId
...
const isBlocked = async (userId) => {
  ...
  return { blocked: true, reason: data.reason, expiresAt: data.expires_at };
};

/**
 * Desbloqueo manual por un administrador (HU-03, criterio extra).
 * El desbloqueo automático lo hace Redis al expirar el TTL.
 * @param {string} userId
 */
...
module.exports = { blockUser, isBlocked, unblockUser, BLOCK_TTL };